...
| 日期 | 变更内容 |
|---|---|
| 2026-08-07 | **本地/云端协同抓取机制梳理与排障（fetch_and_push.sh 全链路）**：(1) 确认云端 .github/workflows/daily_trade.yml 里已有"本地成功则云端跳过"机制（36b8e7e，2026-08-06引入）——定时触发(schedule)前先检查最近20小时内是否有本地推送时打的 "本地补充 ETF 价格数据" commit，命中则跳过装依赖/跑分析/推送/部署 gh-pages 全部步骤（仅对schedule生效，手动 workflow_dispatch 不受影响），本地/云端不会重复跑。(2) fetch_and_push.sh 本身改造：原设计是"网络检测→git pull→跑抓取脚本→git push"一条链，网络检测放在最前面，导致网络不通/VPN未连时本地抓取直接不跑（此前发生过：连续5次网络检测失败，本地数据完全没抓）；改为"本地抓取优先（不依赖github网络）→抓取成功且数据有变化才检测网络→pull/push"，git push 连续3次失败也不再报错阻塞，因为用户日常用 GitHub Desktop 手动 pull/push，本脚本现在只需保证本地数据抓取本身可靠，网络/推送环节失败会给出明确提示（退出码2，"请用 GitHub Desktop 手动推送"）而不是笼统失败。(3) 排查出两个独立本地环境问题：① py_mini_racer(akshare内部V8引擎) 在 MAX_WORKERS=2 并发下于本地macOS环境偶发崩溃（[FATAL:address_pool_manager.cc] Check failed: !pool->IsInitialized()），改 MAX_WORKERS=1 串行后未再复现（a4365e4）；② ak.fund_etf_fund_info_em() 报 pandas "Length mismatch: Expected axis has 14 elements, new values have 13 elements"，导致 latest_discount_rate/discount_quantile_1y 全部为NaN，根因是东方财富接口historical净值返回体新增字段、而本地 akshare 版本(1.18.82)落后，PyPI 已发布 1.18.83 且 GitHub akfamily/akshare 仓库 docs/changelog.md 明确记录过"修复 fund_etf_fund_info_em 接口因东方财富历史净值返回体新增字段而触发 Length mismatch 的问题"，用 `python3 -m pip install --upgrade akshare`升级（务必用脚本实际调用的解释器完整路径，不要用默认全局pip，否则升级不会对脚本生效）后问题消失。**排障方法论备忘：下次再遇到 akshare 接口报错（尤其 Length mismatch / 字段对不上），第一步先查 https://pypi.org/project/akshare/ 是否有更新版本 + 用 `repo:akfamily/akshare 关键词 type:code` 或直接搜 docs/changelog.md 确认是否已知问题，往往比排查自己代码更快。** (4) 注意事项：本地代码若通过 GitHub 网页/Desktop 直接修改（而非本地改完再推），必须先 pull 到本地才能跑最新代码，否则会出现"GitHub上已修复但本地报错"的错位（本次曾因此误以为 `_load_previous_snapshot` 函数丢失，实际只是本地未同步 ce8d237 提交）。涉及文件：fetch_and_push.sh、fetch/simple_etf_metrics.py、.github/workflows/daily_trade.yml（仅确认未改动） |
...
| 2026-08-04 | （未成功解决）曾尝试修复 ETF 价格数据自 8/2 起冻结不更新的 bug，搭建本地定时抓取自动化方案作为补充；但云端 Actions 抓取失败的根因当时未解决，问题延续到 08-06/08-07 才通过 fetch_and_push.sh 改造 + akshare 升级 + MAX_WORKERS 调整实际排查清楚，详见 2026-08-07 记录 |
