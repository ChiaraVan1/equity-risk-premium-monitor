...
# ══════════════════════════════════════════════════════════════════════
#  📋 代码级冗余排查总览（2026-07 by Claude，按用户优先级处理进度）
#  ------------------------------------------------------------------
#  ✅ 已删除：死import(hashlib/timedelta/Path)、死函数
#             (current_erp_percentile_for_signal)、死字典(_FUND_ICON)
#  ✅ 已重构：赔率/胜率计算合并为一次（analyze_and_suggest 算好后传给
#             build_unified_valuation_block，不再各自独立算一遍，
#             顺带修复了SPY/QQQ/EWQ/EWG/EWJ锚定子集口径不一致的问题）；
#             止损/止盈信号的价格MA计算合并为一次（新增 _compute_ma_stats
#             辅助函数，compute_*_signal_summary 和 build_*_signal_block
#             共用同一份 MA20/MA60/MA120，不再各自重新算）
#  ❌ 更正：build_trend_block 的月度趋势斜率和 compute_erp_slope_signal 的
#             日度斜率经复核后确认【不是重复计算】——分别是10个月月度趋势
#             和20个交易日日度斜率，时间尺度不同，予以保留，不合并
#  ⏸ 暂缓（按你的优先级，先确认用途再处理）：
#     summary_list 里 odds / etf_signal / slope_signal / fundamental_*
#     四个未读取字段 —— 保留，等你确认是否要展示或删除
#  ⏸ 暂不动：exit_verdict_icon/exit_signal、profit_verdict_icon/profit_signal
#     两组重复取值字段
# ══════════════════════════════════════════════════════════════════════
...
def _load_shiller():
    """
    读取并缓存 Shiller CAPE 原始数据文件（./data/ie_data.xls），
    按 CAPE 区间分组，预计算每组的历史超额回报统计量。
    只在首次调用时读盘+计算，之后复用 _shiller_cache 全局缓存。
    返回：(分组统计表 grouped, 全量有效样本 valid, 当前最新 CAPE 值 cape_now)
    仅供 build_shiller_block 内部调用，只有 SPY 会用到。
    """
    if _shiller_cache:
...
def build_shiller_block(code):
    """
    Shiller CAPE 长期回报锚区块，只对 SPY 生效（其余标的直接返回空字符串）。
    用美股150年历史数据，按当前CAPE所在区间匹配历史同区间的10年超额回报分布，
    作为长期回报预期的参考锚点（和ERP框架是互补关系，不是替代）。
    """
    if code != "SPY":
        return ""
...
def load_ps_data():
    """
    读取 HSTECH 专用的 PS/PSY 数据文件（./data/ps_HSTECH.csv）。
    HSTECH 的 ERP/PE 框架已停用，改用营收口径的 PS/PSY 代替，
    因为港股科技股盈利波动大，PE容易失真，PS相对稳定。
    文件不存在时返回 None，调用方需要自行判断降级处理。
    """
...
def calc_odds(cur_val, val_series):
    """
    计算"赔率"（盈亏比）：以历史P10/P90作为下沿/上沿锚点，
    upside = 当前值 - P10（估值继续变贵的空间，即"最多能赚多少"的估值锚）
    downside = P90 - 当前值（估值修复的空间）
    注意：这里的"贵/便宜"方向取决于传入的是ERP还是PE系列，
    调用方需保证 val_series 是"越高越便宜"的指标（如ERP、PSY）。
    downside<=0 时返回 None（已经超出历史P90，视为极端低估，赔率"无限大"）。
    upside<=0 时返回 0.0（已经跌破P10，视为无赔率可言）。
    """
...
def compute_erp_slope_signal(erp_series: pd.Series) -> dict:
    """
    计算"近20日ERP变化"的斜率信号（🚨恐慌踩踏/🟢估值快速改善/🟡横盘震荡/
    🟠估值快速恶化/⚠️情绪过热五档）。

    2026-07改动：判断"是否算极端变化"不再用固定的绝对阈值(_SLOPE_EXTREME_THRESHOLD=2%)，
    而是优先用该标的自己历史上所有"20日变化"的分布来算当前变化的分位数。
    原因：ERP=1/PE-rf是倒数关系，PE越高（如百倍PE的题材股），ERP绝对值的
    变动空间天然被压缩，固定2%阈值对这类标的可能永远触发不了；
    改成"和自己历史比"之后，同一套代码对低PE和高PE标的都能自适应识别异常。
    历史样本不足_SLOPE_MIN_HISTORY(60)条时，退回原来的固定阈值兜底。
    """
...
def _load_etf_price_series(erp_code: str):
    """
    从 ./etf_price.csv 里取出指定标的的价格序列（全局按列缓存 _ETF_PRICE_CACHE，
    避免21个标的每个都重新读一次CSV）。
    该CSV由另一个仓库的 simple_etf_metrics.py 生成后同步过来，
    本仓库自己不产生这份数据。文件不存在或标的没有对应列时返回 None，
    调用方（止损/止盈模块）需要自行处理"无数据"的降级展示。
    """
...
def _compute_ma_stats(price_s: pd.Series):
    """
    从价格序列计算 MA20/MA60/MA120，供止损信号（compute_exit_signal_summary）
    和止盈信号（compute_profit_signal_summary）共用一份实现，
    避免两边（以及各自的 build_*_signal_block 详情函数）各自重复计算一遍。
    样本不足时对应位置返回 np.nan，由调用方自行判断降级展示。
    """
    n = len(price_s)
    ...
    计算减仓/清仓信号的核心结论，返回结构化字典（用于置顶区域和详情区域共享）：
      {
        ...
        "has_data": True/False,
        # 2026-07新增：把中间计算结果也一并返回，供 build_exit_signal_block
        # 直接复用，避免重新加载价格+重新算一遍dd/MA（原来是两处独立计算同一件事）
        "cur_price", "recent_high", "dd", "dd_pct",
        "ma20", "ma60", "below_ma20", "below_ma60",
      }
...
def build_exit_signal_block(erp_code: str, current_erp_percentile: float) -> str:
    ...
    2026-07重构：cur_price/recent_high/dd/dd_pct/ma20/ma60/ma120/below_ma20/
    below_ma60 现在全部直接读取 compute_exit_signal_summary 返回的字典，
    不再重新调用 _load_etf_price_series + 重新算一遍完全相同的dd/MA
    （原来这里和 compute_exit_signal_summary 各自独立计算了一遍，
    两处理论上该输出同一个值，属于纯粹的重复运算）。
    只有 QQQ 单日急跌区块仍需要单独读一次 price_s 拿"今日/昨日收盘价"，
    这两个数值不属于止损判断的一部分，summary 里没有保存。
    """
...
def compute_profit_signal_summary(erp_code: str, current_erp_percentile: float) -> dict:
    """
    止盈信号，结构镜像 compute_exit_signal_summary：
    用"现价相对均线的乖离率"代替"从高点回撤"，
    乖离率越高代表短期涨幅越极端、越应该考虑落袋。
    若同时处于高估区（估值分位<_TP_OVERVALUED_PCTL），基础动作升一级（止盈更激进）。

    2026-07重构：MA20/MA60计算改用共用的 _compute_ma_stats（和止损信号同一份
    实现），并把 cur_price/ma20/ma60/dev20/dev60 一并放进返回字典，
    供 build_profit_signal_block 直接复用，不必重新加载价格+重新计算一遍。
    """
    price_s = _load_etf_price_series(erp_code)
...
def build_profit_signal_block(erp_code: str, current_erp_percentile: float) -> str:
    """
    止盈信号详情区块，结构镜像 build_exit_signal_block。

    2026-07重构：cur_price/ma20/ma60/dev20/dev60 直接读取
    compute_profit_signal_summary 返回的字典，不再重新加载价格+重新算一遍
    （原来这里和 compute_profit_signal_summary 各自独立计算了一遍MA/乖离率）。
    """
...
def build_fundamental_alert_block(code: str, name: str) -> tuple[dict, str]:
    """
    基本面暴雷预警主流程：
      1. 宽基/国别指数（_SKIP_FUNDAMENTAL_CODES）直接跳过，不适用单一暴雷逻辑
      2. 本地关键词粗筛东方财富快讯（_em_news_search，允许假阳性）
      3. 没命中任何新闻 → 硬性短路，不让AI"脑补"，直接标记"未知"
      4. AI第一遍：相关性过滤，剔除粗筛误报的新闻
      5. AI第二遍：对过滤后的新闻做暴雷判断 + 逐条正负面分类
    返回 (结构化结果字典, 供报告拼接的markdown文本)。
    调用两次AI接口（步骤4/5），是本文件里唯一真正花钱花时间的模块，
    其余信号（止损/止盈/斜率/ETF折溢价）都是纯本地pandas计算。
    """
...
def build_unified_valuation_block(df, code, val_series=None, win_rate=None, odds_ratio=None):
    """
    生成"核心估值决策"区块：展示胜率（历史分位）+ 赔率（P10/P90盈亏比），
    综合评级（🟢🟡🟠🔴🚨）。
    HSTECH走PS/PSY口径，其余标的走ERP/PE口径，字段名统一用 m_name/p_name 占位。

    2026-07重构：val_series / win_rate / odds_ratio 现在由调用方
    （analyze_and_suggest）算好后传入，本函数不再自己重新计算一遍。
    这不只是去重复，还修了一个真实的口径不一致问题：SPY/QQQ/EWQ/EWG/EWJ
    这5个标的在 analyze_and_suggest 里用的是"2022年后锚定子集"计算胜率，
    但本函数原来是用未锚定的完整历史序列重算，导致同一份报告里
    上方"估值区间"和这里的"胜率"可能对不上。现在统一用调用方传入的
    同一份 val_series，两处口径保证一致。
    若调用方未传入（如单独调试），退回自己按完整历史计算的旧逻辑。
    """
...
def build_trend_block(df, erp_series, code, quantiles):
    """
    生成"近10月趋势"表格区块（HSTECH走PSY月度趋势，其余走ERP月度趋势），
    并在表格上方附带斜率信号（compute_erp_slope_signal）的判断结果。

    2026-07更正：这里内部用 recent['ERP'] 手动算的 slope（np.polyfit）
    和 compute_erp_slope_signal 内部的斜率**不是重复计算**——
    本函数算的是"近10个月"月度数据的趋势方向（用于"持续走高/走低/基本横盘"文案），
    compute_erp_slope_signal 算的是"近20个交易日"日度数据的斜率（用于信号分级）。
    两者时间尺度和数据粒度都不同，回答的是不同问题，合并会把两个周期的信号
    错误地混成一个，所以保留两套独立计算，不做合并。
    """
    if code == "HSTECH":
        ps_df = load_ps_data()
...
def is_holding(code: str) -> bool:
    """
    判断某个标的当前是否为实际持仓（读 HOLDING_CATEGORY 字典）。
    仅用于仪表盘展示📌徽章，不影响任何计算逻辑。
    """
...
def generate_action_sentence(disc, divg, vol, zone_label):
    """
    把折溢价(disc)/量价背离(divg)/波动率(vol)三个ETF执行面信号，
    拼成一句给"未触发止损/止盈"的正常标的看的操作建议文案
    （仅用于仪表盘常规分组里的展示行，不涉及仓位百分比计算）。
    """
...
def markdown_to_html(md_text: str, date_str: str) -> str:
    """
    极简的自制 Markdown → HTML 转换器（不依赖第三方markdown库），
    只支持本项目实际用到的语法子集：标题(#/##/###/####)、引用(>)、
    表格(|...|)、分割线(---)、粗体/斜体/行内代码、代码块(```)。
    仅供 save_html_report 内部调用，生成最终推送的 report.html。
    """
    import re
...
def save_html_report(full_report_md: str, date_str: str):
    """
    把完整报告的 Markdown 转成 HTML 并写入 ./docs/report.html，
    供 GitHub Pages（gh-pages分支）部署展示。
    """
...
def send_to_wechat(summary_md: str, date_str: str):
    """
    通过方糖（Server酱）把仪表盘摘要（markdown格式）推送到微信。
    未配置 SCT_KEY 环境变量时直接跳过（不报错），方便本地/dry-run调试。
    """
...
def analyze_and_suggest(code, name, etf_df=None, summary_list=None):
    """
    单个标的的主分析函数，会被 __main__ 里的循环对21个标的逐个调用。
    职责很重，串起了几乎所有模块：
      1. 读取 ./data/erp_{code}.csv，计算ERP历史分位与仓位建议(b/v/t三段仓)
      2. HSTECH 特殊分支：整体换用 PSY 数据覆盖 ERP 计算结果
      3. 依次调用：ETF折溢价信号 → 斜率信号 → 止损信号 → 止盈信号 →
         人气热度信号 → 基本面暴雷预警
      4. 把所有信号汇总写入 summary_list（供仪表盘置顶区/分组展示用）
      5. 拼接所有区块生成该标的的完整 markdown 报告并返回
    注意：这个函数超过200行，各信号计算+区块拼接全挤在一起，
    如果要精简代码，这里是优先考虑拆分的地方
    （比如拆成 _compute_all_signals() + _build_report_markdown() 两半）。
    """
    ...
    if summary_list is not None:
        # ⚠️ 冗余排查提示：以下字段全文搜索后确认【从未被任何下游代码读取】
        # （build_summary_block 和 __main__ 排序逻辑都没用到）：
        #   odds, etf_signal, slope_signal,
        #   fundamental_alert, fundamental_positive, fundamental_negative,
        #   fundamental_sentiment_available
        # 注：win_rate 不在此列——它被 __main__ 底部 _sort_key() 用于仪表盘排序，不是死字段。
        # 另外 etf_signal 的值本身就是 _etf_discount_signal 的重复拷贝（见上面赋值处，
        # 两个变量永远相等，从未分叉过）；exit_verdict_icon 和 exit_signal、
        # profit_verdict_icon 和 profit_signal 同理，都是同一个值存了两份key。
        # 仪表盘（build_summary_block）实际会读取的字段只有：
        #   code, erp_zone, total_pct, b_pct/v_pct/t_pct, etf_discount,
        #   etf_divergence, etf_vol, exit_level, exit_verdict_line,
        #   profit_level, profit_verdict_line, popularity_icon
        # 如果确认这些字段未来也不打算展示，可以考虑精简掉。
        summary_list.append({
            "name": name, "code": code,
            "erp_zone": erp_zone,
            "total_pct": total_pct,
