...
def compute_exit_signal_summary(erp_code: str, current_erp_percentile: float, holding: bool = True) -> dict:
    ...
    if max_level == 0 and is_protected:
        ...
        # 但仍要让它出现在报告里——这正是修复"未持仓就完全消失"的关键。
        icon_map = {1: "🔎", 2: "🔎", 3: "🔎"}
...
def build_exit_signal_block(erp_code: str, current_erp_percentile: float, holding: bool = True) -> str:
    """止损信号详情区块（含表格），直接复用compute_exit_signal_summary算好的数值。"""
    summary = compute_exit_signal_summary(erp_code, current_erp_percentile, holding=holding)
    ...
    if output_format == "markdown":
        ...
        if alerted:
            ...
            for r in alerted:
                ...
                lead_icon = ("🔎" if not is_holding(r["code"]) else "🚨") if r.get("exit_level", 0) > 0 else "💰"
                lines.append(
        ...
        if alerted:
            ...
            for r in alerted:
                ...
                profit_line = r.get("profit_verdict_line", "") if r.get("profit_level", 0) > 0 else ""
                combined = "；".join(x for x in (exit_line, profit_line) if x)
                lead_icon = ("🔎" if not is_holding(code) else "🚨") if r.get("exit_level", 0) > 0 else "💰"
                rows_html.append(
                    f'<tr class="alert-row">'
...
未持仓标的的止损信号改为🔎观察提示（不建议操作，仅供参考），不再因未持仓而完全跳过计算
...
def analyze_and_suggest(code, name, etf_df=None, summary_list=None):
    ...
    # ── 减仓信号（不管持不持仓都算：未持仓也需要能看到回撤已触线）──────
    # ── 止盈信号仅对持仓有意义，未持仓无仓位可止盈，维持跳过 ──────────
...
if __name__ == "__main__":
    ...
    if report_list:
        date_str       = datetime.now().strftime("%Y-%m-%d")
        summary_html   = build_summary_block(summary_list, output_format="html")
>>>>>>> e1b472e19c7e1779136686670cd00d09f94e70fe
