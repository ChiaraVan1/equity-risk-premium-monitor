...
```
若 P90 − 当前ERP ≤ 0（已超P90极度低估） → 赔率视为"极高"
...
宽基/国别指数（`000300` `000688` `SPY` `QQQ` `EWQ` `EWG` `EWJ` `EEM`）成分股高度分散，本模块不适用，直接跳过。其余标的（含军工、稀土、半导体、有色金属等主题指数）各有专属关键词表，用于本地粗筛。
...
`data/simple_etf_metrics.csv` 每日生成并随commit写回本仓库（不再依赖另一 Repo Release）。数据源已从 Tushare 切换为纯 AKShare（不再需要 `TUSHARE_TOKEN`）：
