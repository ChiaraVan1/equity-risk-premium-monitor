...
| `GH_PAT` | GitHub Personal Access Token，供 Claude Cowork 调用 GitHub REST API（如写入 QQQ_PE_TODAY Variable）。抓取仍用 Chrome MCP 浏览网页，但写入已改为直接 API 调用，不再模拟网页表单操作，详见 [STATUS.md](./STATUS.md) 的「QQQ PE 抓取机制」 | 是 |
